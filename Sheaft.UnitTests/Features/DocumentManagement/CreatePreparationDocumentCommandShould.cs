...
public class CreatePreparationDocumentCommandShould
{
    ...
    public async Task Insert_Document_Waiting_For_Processing()
    {
        ...
        var orderIdentifiers = context.Orders.Select(o => o.Id).ToList();
        
        var result = await handler.Handle(new CreatePreparationDocumentCommand(orderIdentifiers, new OwnerId(supplier.Id), true),
            CancellationToken.None);

        Assert.IsTrue(result.IsSuccess);
        var document = context.Documents.Single(d => d.Id == new DocumentId(result.Value));
    ...
    public async Task Fail_If_Order_Not_Found()
    {
        ...
        var result = await handler.Handle(new CreatePreparationDocumentCommand(orderIdentifiers, OwnerId.New(), true),
            CancellationToken.None);
    ...
    public async Task Fail_If_Order_Not_Accepted_With_AutoAcceptOrders_False()
    {
        ...
        var result = await handler.Handle(new CreatePreparationDocumentCommand(orderIdentifiers, new OwnerId(supplier.Id)),
            CancellationToken.None);
