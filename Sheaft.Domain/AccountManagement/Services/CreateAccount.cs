...
{
    Task<Result<Account>> Create(Username username, EmailAddress email, NewPassword password, CancellationToken token);
}
...
internal class CreateAccount : ICreateAccount
{
    ...
    public async Task<Result<Account>> Create(Username username, EmailAddress email, NewPassword password, CancellationToken token)
    {
        ...
        return Result.Success(new Account(username, email, hashedPassword));
    }
}