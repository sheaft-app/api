...
public record GetOrderDraftQuery(OrderId Identifier, AccountId AccountId) : IQuery<Result<OrderDraftDto>>;
...
internal class GetOrderDraftHandler : IQueryHandler<GetOrderDraftQuery, Result<OrderDraftDto>>
{
    ...
    {
        return await _orderQueries.GetDraft(request.Identifier, request.AccountId, token);
    }
}