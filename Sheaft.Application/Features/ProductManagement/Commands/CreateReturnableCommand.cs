...
public record CreateReturnableCommand(string Name, string Code, decimal Price, decimal Vat,
    SupplierId SupplierIdentifier) : Command<Result<string>>;
...
internal class CreateReturnableHandler : ICommandHandler<CreateReturnableCommand, Result<string>>
{
    ...
    {
        var referenceResult =
            await _handleReturnableCode.ValidateOrGenerateNextCode(request.Code, request.SupplierIdentifier, token);
