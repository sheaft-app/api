...
namespace Sheaft.GraphQL.DeliveryModes
{
    public class DeliveryHoursByIdBatchDataLoader : BatchDataLoader<Guid, Domain.DeliveryHours>
    {
        ...
        protected override async Task<IReadOnlyDictionary<Guid, Domain.DeliveryHours>> LoadBatchAsync(
            IReadOnlyList<Guid> keys,
            CancellationToken token)
        {
            return await _contextFactory.CreateDbContext().Set<Domain.DeliveryHours>().Where(u => keys.Contains(u.Id))
