    ...
    public class PictureService : BaseService, IPictureService
    {
        ...
        public async Task<Result<string>> HandleProductPictureAsync(Product entity, string picture, CancellationToken token)
        {
            return await ExecuteAsync(async () =>
            {
                ...
                {
                    await UploadPictureAsync(image, entity.Producer.Id, entity.Id, imageId, PictureSize.LARGE, 620, 256, token, quality: 60);
                    await UploadPictureAsync(image, entity.Producer.Id, entity.Id, imageId, PictureSize.MEDIUM, 310, 128, token, quality: 80);
