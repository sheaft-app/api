<script lang='ts'>
  ...
  import { ProposeAgreementToCustomerRequest } from '$commands/customers/proposeAgreementToCustomer'
  ...
  const validate = async () => {
    try {
      await mediator.send(new ProposeAgreementToCustomerRequest(customer.id, deliveryDays, orderDelayInHoursBeforeDeliveryDay))
    } catch (exc) {
      console.error(exc)
    }
  }  
...
<div>
  ...
  <ul class='m-3'>
    <li><label class='form-check-label inline-block text-gray-800 cursor-pointer'>
  ...
  <div class='mt-5 mb-8'>
    <Checkbox id='orderDelay' bind:value='{orderDelayed}'
              label='Verrouiller la prise de commandes x heures avant le jour de livraison' />
    {#if orderDelayed}
      <Number label="Nombre d'heures" bind:value={orderDelayInHoursBeforeDeliveryDay} class='mt-2' />
    {/if}
  </div>
</div>
