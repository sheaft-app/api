...
import type { IRegisterAccount, IConfigureCustomer, IConfigureSupplier, IUser } from '$types/auth'
...
export const isRegistered = derived(
  [userStore],
  ([$userStore]) => $userStore.isAuthenticated && $userStore.account.profile.status == ProfileStatus.Registered
)
...
export const configureSupplier = async (account: IConfigureSupplier): Promise<boolean> => {
  return configure(account, ProfileKind.Supplier);
}

export const configureCustomer = async (account: IConfigureCustomer): Promise<boolean> => {
  return configure(account, ProfileKind.Customer);
}

const configure = async (account: IConfigureCustomer, type:ProfileKind): Promise<boolean> => {
  try {
    await axios.post(`/api/account/configure/${type == ProfileKind.Customer ? 'customer' : 'supplier'}`, account)
