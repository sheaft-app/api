<script lang="ts">
  ...
  import { RefuseOrderCommand } from "$components/Orders/commands/refuseOrder";
  ...
  const validate = async () => {
    try {
      const result = await mediator.send(new RefuseOrderCommand(order.id, reason));
      close();
...
<h2 class="mb-4">Refuser la commande n°{order.code}</h2>
<hr />
<div class="my-6">
  <p class="my-4">
    Vous vous apprêtez à refuser la commande de {targetName}.
  </p>
  <p class="my-4">
    Veuillez préciser la raison de ce refus pour permettre à {targetName} de comprendre votre choix.
  </p>
...
<div class="flex items-center justify-evenly pt-4">
  ...
  <Button class="bg-danger-600" on:click="{validate}">Valider</Button>
</div>
