<script lang='ts'>
  ...
  onMount(() => {
    if(agreement.status == AgreementOwner.Customer) {
...
<hr />
<div class='my-6'>
  <p>Vous vous apprêtez à accepter la demande d'accord commercial proposé par {proposedBy}. {actionStr}.</p>
...
<div class='flex items-center justify-evenly pt-4'>
  ...
  <Button class='bg-accent-600' on:click='{validate}' disabled='{!reason || reason.length < 1}'>Accepter</Button>
</div>
