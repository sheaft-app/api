  ...
  const validate = async () => {
    try {
      const result = await mediator.send(
        new ProposeAgreementToSupplierCommand(supplier.id)
      );
      close();
...
<div class="my-6">
  <p>
    Vous allez envoyer une demande de mise en relation commerciale avec le producteur
    suivant :
  </p>
  <div class="mx-3 my-5">
    <p><b>{supplier.name}</b></p>
    <p>{@html address(supplier.shippingAddress)}</p>
  </div>
  <p class="mt-4">
    Ce producteur assignera des jours de livraison en fonction de ses disponibilités au
    moment d'accepter votre demande
  </p>
</div>
<hr />
<div class="flex items-center justify-evenly pt-4">
  <Button class="bg-default-600 mx-2" on:click="{close}">Fermer</Button>
