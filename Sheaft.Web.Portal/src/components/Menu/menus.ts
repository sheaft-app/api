...
const parseAndAssignToMenus = (children: ClientNodeApi[], menus: Record<string, MenuGroup | MenuEntry>, parent?: string, roles?: string[]) => {
  children.forEach((c: ClientNodeApi) => {
    ...
    if (c.children?.length > 0) {
      ...
      (<MenuGroup>menus[menuName]).pages.push(<MenuEntry>getMenuElement(c, null, menus[menuName], menus[menuName].roles))
    }
    else {
...
const getMenuElement = (route: ClientNodeApi, title?: string | null, parent?: MenuGroup, roles?: string[], isGroup:boolean = false): MenuElement => {
  if (!isGroup)
    return <MenuEntry>{
      path: route.path,
...
export const canHighlightMenuItem = (entry: MenuEntry, currentPath: string): boolean => {
  ...
  if (entry.parent)
    ...
    return (
      entry.parent.pages.find((p: MenuEntry) => !p.visible && p.path == currentPath) != null
    )
