<script lang="ts">
  ...
  import { canDisplayMenuElement, canHighlightMenuItem, entryIsGroup, entryIsPage } from '$components/Menu/menus'
  ...
  const isPage = (entry:MenuElement):boolean => {
    if(entryIsPage(entry))
      return !(<MenuEntry>entry).default;
